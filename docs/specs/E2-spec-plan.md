...
> ✅ **已答（`E2-brush.md §2.1`）：都不是——程序生成，不存檔。** 這**推翻了上面
> 「已拍板」表的 tip 來源列**。value noise 的三個常數（seed／頻率／對比）就是可調的
> 那部分，存檔只多一道「生成器與產物是否同步」的 CI 題。
...
> ✅ **已答（`E2-brush.md §6.1`）：E1 的實作已經避開了。** `emit()` 無條件抽四個值，
> jitter 為 0 也照抽——從 0 啟用不改變 RNG 的串流節奏。剩下的是排序約束，寫成
> `§6.2` 的四條契約並由 `properties.rs` 現在就守著（不必等 D5）。
>
> **實作時發現一條漏網的**：`jitter_angle = 0` 時抽到負值會得到 `-0.0`，與 `0.0`
> 數值相等但位元不同，於是「三欄全 0 逐位元相同」會敗在一個看不見的符號位。
> 已在 `builder.rs` 的 `emit()` 收正。

**必答 C**｜`TipId::is_implemented()` 的 fallback 在 E2 之後留不留。三個變體全實作後
它恆為真，留著是死碼，拿掉則 `dab.rs` 的 fallback 分支一併移除。

> ✅ **已答（`E2-brush.md §2.2`）：移除，兩者一併。** 留著不只是死碼——日後新增
> 第四張 tip 卻忘了填 atlas 時，它看起來像有保護、實際上是靜默 fallback 成軟圓。
> 移掉之後缺層就是明顯的空白筆跡。已實作。
...
| **tip 來源** | ~~顆粒是內嵌 PNG~~ → **三張全程序生成**，不進 `assets/`／LFS／baker | **本列已被 `E2-brush.md §2.1` 推翻。** 原理由是「顆粒的價值在不規則，而不規則參數調不動」——這對 value noise 不成立：它的不規則性由 seed、頻率、對比三個常數完全控制，改一個就重跑，正是同一張表給軟圓與硬圓的理由。顆粒又是五支裡最需要反覆調的一張，PNG 會讓每次調校都是一份二進位進 git。三張都是程式碼常數，不是文件資產 |
