...
每張線稿交付**三個必交 PNG（`lineart` / `flats` / `reference`）＋ 一個選配（`shade`）＋ 一份 `meta.json`**。所有 PNG 必須同尺寸、同對齊，且**從同一個來源檔的不同圖層導出**——分別新建畫布必然錯位。

| 檔案 | 內容 | 硬性要求 |
|---|---|---|
| `lineart.png` | 線稿，透明背景 | **抗鋸齒開啟**，RGBA，背景須為真透明（導出時不得填白） |
| `flats.png` | 塗り分け（分色圖），每個封閉區域填**單一純色**；不相鄰的區域可重用同色 | **抗鋸齒關閉**（硬邊）、無漸層、無縫隙、**不透明且全覆蓋**（見 §9.3）、不得使用保留色 `#FF00FF` |
| `reference.png` | **建議配色稿**——`flats` 的換色版：每區填的是真正的建議顏色 | 同 `flats`（抗鋸齒關閉、不透明、全覆蓋），且**不得引入 `flats` 沒有的邊界**（相鄰區同色使邊界減少是允許的） |
...
| 檔案 | 濾波器 | 為什麼 |
|---|---|---|
| `flats` 的 **region ID map** | **2×2 majority（取眾數）** | 不得引入新顏色。純 NEAREST 也不引入新顏色，但它取的是每個 2×2 的左上角像素——**細於 2px 的區域會隨機消失**，取決於它落在哪個格子。majority 保留細區域的能力好得多，成本可忽略 |
...
| `flats` 唯一色數 > **1024**——快篩 | 母帶 | 錯誤 |
| `flats` 任一顏色總面積 < **100px**——實判，這才是「無抗鋸齒」的判準 | 母帶 | 錯誤 |
| `flats` 使用保留色 `#FF00FF` | 母帶 | 錯誤 |
| **無未指派像素（無例外，含線稿覆蓋處）**。定義：`flats` 的 **alpha < 255** | 母帶 | 錯誤 |
...
**面積門檻分屬不同解析度**，這是最容易混用的地方：`flats` 顏色面積的 100px 在**母帶**、碎片區域的 200px 在**輸出**（繪師端對應的母帶數字是 800px）。

**唯一色數不能取代面積判準。** 前者是快篩——命中代表圖徹底壞掉（例如色彩空間被轉換過），可以在算面積直方圖之前就停；後者才是 `specs/assets-spec.md §7` 對繪師承諾的那條。`adventure-time-demo-1/flats.png` 有 171 個顏色、其中 166 個總面積 < 200px，唯一色數遠低於 1024——只跑快篩會把一張已知不合規的素材放行。
...
| `reference.png` | `palette[]`、`regions.json[].suggested_color` |
| baker 推導 | `schema_version`、`content_hash`、`canvas_size`、`aspect`、`region_count`、`difficulty`、`has_shade` |

**`palette[]` 與 `suggested_color` 不是兩份真相，是兩個東西**（兩者都源自 `reference.png`，但服務不同的地方）：
